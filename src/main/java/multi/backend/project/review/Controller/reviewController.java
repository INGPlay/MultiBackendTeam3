...
//  게시글 전체 출력
    ...
    public String reviewForm(Model m, HttpServletRequest seq, HttpSession session){
        // @RequestParam("pageVo") pagingVO pageVo
        System.out.println((pagingVO)session.getAttribute("pageVO"));;
...
//    게시글 수정&삭제 폼 이동
    ...
    @PostMapping("/update")
    public String updateReview(Model m, @ModelAttribute reviewVO vo,HttpSession session){
        pagingVO page = (pagingVO) session.getAttribute("pageVO");
        int select = page.getCri().getSort();
