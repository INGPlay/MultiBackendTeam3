...
- 위 그림 1번은 각 메인 페이지, [지도 페이지](#2-지도-페이지), [리뷰 페이지](#3-리뷰-페이지), [팀 페이지](#4-팀-페이지)로 이동하는 링크를 나타낸다.
- 위 그림 2번과 같이 [로그인 페이지](#5-로그인-페이지) 및 [가입 페이지](#6-가입-페이지)로 이동하는 링크를 나타낸다.
- 위 그림 3번은 로그인 여부에 따라 [마이 페이지](#7-마이-페이지) 및 로그아웃 버튼을 나타냄을 보여준다.
- 위 그림 4번은 어드민 계정 로그인에 따라 [관리자 페이지](#8-관리자-페이지)로 이동하는 버튼을 추가적으로 나타냄을 보여준다.
...
- 메인 페이지는 영상을 반복 재생된다.
- 위 그림 1번과 같이 [로그인 페이지](#5-로그인-페이지) 및 [가입 페이지](#6-가입-페이지)로 이동하는 링크를 나타낸다.
- 위 그림 2번은 로그인 여부에 따라 로그아웃 버튼이 나타냄을 보여준다.

### 1-3. 푸터 (1. 메인페이지 그림 3번)
![footer](images/footer_draw.png)
- 위 그림 1번의 링크를 클릭하면 이 리포지토리 페이지로 이동한다.
...
- 게시된 지도를 나열한다.
- 위 그림 1번은 이 지도의 출발지역과 도착지역을 나타낸다. (출발지역 → 도착지역)
- 위 그림 2번은 [지도 게시글 페이지](#2-b-지도-게시글-페이지)로 이동하는 버튼을 나타낸다.
...
- 위 그림 그림 1번은 검색할 카테고리를 선택한다.
- 위 그림 2번은 검색할 단어를 입력한다.
...
- <b>지도에서 장소를 찾는 방법에는 2가지가 있다.</b>
  - 지도 내에서 마우스로 클릭한 좌표 근처에 마커가 표시된다.
  - 위 그림 2번 검색창으로 지역이나 장소로 검색하여 조건에 맞는 마커가 표시된다.
- 위 그림의 1번의 버튼으로 검색할 카테고리를 선택할 수 있다.
- 위 그림의 3번은 마커를 직접 클릭하면 나타나는 창이다.
...
- 리뷰 조회 페이지 그림 3번의 게시글 조회 순서 기준을 선택한다.
  - 최신순 : 작성일 기준으로 조회한다.
  - 조회순 : 조회수를 기준으로 조회한다.
  - 추천순 : 추천수를 기준으로 조회한다. 
